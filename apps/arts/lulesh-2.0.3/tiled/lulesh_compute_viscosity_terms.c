/******************************************************************************
 * LULESH Tiled ARTS Version - Compute Viscosity Terms (Tiled)
...
static void computeViscosityTermsForElement(int iteration, int element_id, luleshCtx *ctx) {
    ...
    vector position_gradient = gradientDataPtrs[curr_buf][element_id]->position_gradient;
    vector velocity_gradient = gradientDataPtrs[curr_buf][element_id]->velocity_gradient;
    ...
    if (volume_derivative > 0.0) {
        ...
        for (int face_id = 0; face_id < 6; face_id++) {
            ...
            if (neighbor_elem >= 0) {
                vector neighbor_vel_grad = gradientDataPtrs[curr_buf][neighbor_elem]->velocity_gradient;
